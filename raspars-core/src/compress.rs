...
mod tests {
    ...
    fn compress_columns_roundtrip() {
        ...
        let compressed = compress_columns(&names, &versions, &sources, &checksums, &deps).unwrap();

        assert_eq!(compressed.streams.len(), 5);
        assert_eq!(compressed.streams[0].label, "names");
        assert_eq!(compressed.streams[1].label, "versions");
        assert_eq!(compressed.streams[2].label, "sources");
        assert_eq!(compressed.streams[3].label, "checksums");
        assert_eq!(compressed.streams[4].label, "dependencies");

        let rec_names =
            deserialize_strings(&decompress_bytes(&compressed.streams[0].data).unwrap());
        let rec_versions =
            deserialize_strings(&decompress_bytes(&compressed.streams[1].data).unwrap());
        let rec_sources =
            deserialize_optional_strings(&decompress_bytes(&compressed.streams[2].data).unwrap());
        let rec_checksums =
            deserialize_optional_strings(&decompress_bytes(&compressed.streams[3].data).unwrap());
        let rec_deps =
            deserialize_dep_lists(&decompress_bytes(&compressed.streams[4].data).unwrap());
